...
export function BlackHoleHeroSection({
  ...
  useEffect(() => {
    ...
    function tick(now: number) {
      ...
      if (!visible || props.current.paused) { lastFrame = now; return; }
    ...
    resize();
    settle(reduced || props.current.paused ? 16 : 1);
    if (!reduced && !props.current.paused) raf = requestAnimationFrame(tick);
